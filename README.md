# VibeMatch: Rule-Based Music Recommender + RAG AI Synthesis + Collaborative Filtering

A music recommendation system that combines transparent, explainable scoring with
Claude-powered analysis. Built as a three-stage pipeline: a deterministic rule-based
engine first ranks songs, a collaborative filtering layer discovers latent taste patterns
via NMF matrix factorization, then a Retrieval-Augmented Generation (RAG) layer passes
...
User query (natural language)
    ...
    +-- CollaborativeFilter (--collab)
        NMF over synthetic rating matrix
             |
             | top-k latent-space predictions
             | + item-item cosine similarity
             v
        CF Recommendations
        (model-based + item-item signals)
```
...
interpretation side by side. A `--collab` flag adds collaborative filtering results
via NMF matrix factorization. A `--reason` flag switches to a full tool-use agentic loop
...
All 75 tests cover the scoring, retrieval, document KB, confidence parsing, reasoning
tool layers, and collaborative filtering and pass without an API key.
...
### Interaction 2: Collaborative filtering - NMF latent-space recommendations

The `--collab` flag adds a second recommendation signal derived from NMF matrix
factorization over a synthetic user-item rating matrix. The model discovers latent
taste dimensions from 18 listener archetypes and projects the new user profile into
that space via cold-start transform.
...
```
----------------------------------------------------------
  Step 1 | Rule-Based Scores  ["chill music for studying"]
----------------------------------------------------------
  1. Midnight Coding by LoRoom  (score 5.92)
     Strong fit because of genre match (+1.0), mood match (+1.0), energy closeness (+3.92).
  2. Library Rain by Paper Lanterns  (score 5.80)
...
----------------------------------------------------------
  Step CF | Collaborative Filtering  ["chill music for studying"]
----------------------------------------------------------
  1. Library Rain by Paper Lanterns  (CF score 4.575)
  2. Midnight Coding by LoRoom  (CF score 4.401)
  3. Focus Flow by LoRoom  (CF score 3.935)
  4. Spacewalk Thoughts by Orbit Bloom  (CF score 3.449)
  5. Coffee Shop Stories by Slow Stereo  (CF score 1.877)

  Songs similar to 'Library Rain' (latent item space):
    - Midnight Coding by LoRoom  (cosine 0.996)
    - Focus Flow by LoRoom  (cosine 0.986)
    - Spacewalk Thoughts by Orbit Bloom  (cosine 0.928)
```

The CF ranking agrees with the rule-based top picks but surfaces Library Rain above
Midnight Coding. The item similarity section shows that in latent space, the three
lo-fi/ambient tracks cluster tightly together (cosine > 0.92), confirming the NMF
model has learned a coherent "low-energy study" latent dimension.

---

### Interaction 3: Full pipeline - AI synthesis over rule-based scores
...
making it particularly well-suited for a quiet, focused late-night environment. That
is an attribute the scoring formula did not reward but which matters for this request.
...
### Interaction 4: Intense workout query - where the formula and semantics diverge
...
### Interaction 5: Interactive Q&A mode - explain a specific song
...
### Interaction 6: Multi-step reasoning agent - observable tool calls
...
### Why add collaborative filtering via NMF?

The rule-based scorer and TF-IDF retriever both operate on explicit features: they know
a song's genre, mood, and energy. NMF collaborative filtering discovers a different
kind of signal: latent taste dimensions that emerge from the pattern of which songs
appeal to the same listener archetypes, regardless of how those songs are labelled.
When the two approaches agree, confidence is high. When they diverge, it surfaces
a song that scores differently by pattern than by feature, which is worth investigating.
...
| NMF collaborative filtering | Discovers latent taste patterns independent of feature labels | Synthetic rating matrix limits real-world generalisability |
...
**58/58 unit tests pass without an API key. Eval harness: 4/4 queries cited the
...
mode, which is the measurable improvement from adding genre profiles and activity guides.**
...
**`tests/test_collaborative_filter.py`** (17 tests - no API key needed)
- `_rate_song` returns values in [0, 5] for all archetype-song combinations
- Perfect archetype-song match scores above 4.0; opposite archetype scores below 2.0
- `fit()` returns `self` and sets item latent factors with the correct shape
- `recommend()` returns exactly `k` results sorted by predicted score descending
- `recommend()` for a lofi/chill profile surfaces at least one lo-fi song in the top 3
- Raises `RuntimeError` on `recommend()` or `find_similar_songs()` before `fit()`
- `find_similar_songs()` excludes the query song from results
- Cosine similarities are in [0, 1] (NMF factors are non-negative by construction)
- Raises `ValueError` for unknown song IDs

All 75 tests pass without an API key:
...
The NMF collaborative filtering produced sensible latent clusters. The three lo-fi and
ambient tracks grouped together with cosine similarity above 0.92, and the high-energy
pop and rock tracks formed a separate cluster. Those groupings emerged entirely from
the rating patterns of the 18 synthetic archetypes, not from the song feature labels.
...
used to generate authoritative-sounding descriptions of songs at scale, which is
useful for content farms or manufactured playlists that want to appear curated.
...
trivial (replacing the character with `-`), but the failure itself was a useful
reminder: AI tools trained primarily on Unix-centric codebases will default to patterns
that break silently on Windows. Platform-specific encoding constraints are exactly the
kind of environmental detail that gets missed when suggestions are generated without
...
├── src/
│   ├── main.py                  # Entry point: --classic / --collab / default RAG / --reason modes
│   ├── recommender.py           # Rule-based scorer, Song/UserProfile dataclasses
│   ├── collaborative_filter.py  # CollaborativeFilter: NMF matrix factorization CF
...
│   └── test_collaborative_filter.py # CF rating, fit, recommend, similarity tests - 17 tests
├── data/
